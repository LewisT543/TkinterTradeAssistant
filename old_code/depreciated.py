...
class TradesWindow:
    def __init__(self, master):
        ...
        # Create buttons:
        self.main_menu_button_names = ['1). Show Trades', '2). Add Trade', '3). Search Trades', '4). Position Size Calc.',
                                        '5). extra_button_2','6). extra_button_3', '7). extra_button_4', '8). extra_button_5']
        row, counter = 0, 0
    ...
    def welcome_screen(self, master):
        ...
        def note_parser(lst):
            ...
            for index, note in enumerate(lst):
                string += f'{index}). {note.title}:\n{note.contents}\n'
            return string

        my_notes = note_parser(self.note_handler.notes)
        self.noteframe = tk.LabelFrame(master, text='My Notes', height=370, width=576, font=('Ariel', 16))
        self.noteframe.place(x=10, y=50)
        self.notes_text = tk.Text(self.noteframe, height=12, width=51, font=('Ariel', 14), bd=0, wrap=tk.WORD)
        self.notes_text.place(x=0, y=0)
        self.balance_frame = tk.Label(master, text='Welcome!               Your Balance: £1000.00', font=('Ariel', 20))
        self.balance_frame.place(x=15, y=10)
